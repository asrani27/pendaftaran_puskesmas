...
class HomeController extends Controller
{
    public function user()
    {
        ...
        $data->getCollection()->transform(function ($item) {
            ...
            $s =  DB::connection($item->puskesmas)->table('t_antrian')->where('tanggal', $item->tgl_daftar)->where('kdPoli', $item->kdPoli)->where('status', 0)->where('pendaftaran_id', '<', $item->id)->count();
